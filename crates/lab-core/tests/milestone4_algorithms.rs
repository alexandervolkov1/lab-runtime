...
fn fixed_and_ramp_references_use_only_monotonic_time() {
    ...
    assert_eq!(
        fixed.value_at(Duration::from_secs(100)).unwrap().value,
        42.0
    );

    let mut up = RampReference::new(0.0, 10.0, 2.0, Unit::CELSIUS, Duration::ZERO).unwrap();
    close(up.value_at(Duration::from_millis(2500)).unwrap().value, 5.0);
    close(up.value_at(Duration::from_secs(20)).unwrap().value, 10.0);

    let mut down = RampReference::new(10.0, -2.0, 3.0, Unit::CELSIUS, Duration::ZERO).unwrap();
...
fn ramp_retune_is_continuous_and_invalid_time_is_atomic() {
    let mut ramp = RampReference::new(0.0, 10.0, 2.0, Unit::CELSIUS, Duration::ZERO).unwrap();
