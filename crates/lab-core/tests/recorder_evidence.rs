...
use lab_core::{
    ...
    Unit, Value, ValueSpec, WriteEffect,
    instrument::{DataInstrumentDefinition, DataParameterDefinition, KnownOperation,
        MetakonBinding, MetakonInstrumentConfig},
    metakon::crc,
    output::{ActuatorId, EvidenceLevel, OutputCommand, OutputOwner, OutputProposal,
...
#[derive(Default)]
struct Wire { bytes: Vec<u8>, readable: VecDeque<u8> }
...
impl ByteTransport for Fake {
    ...
    fn try_read(&mut self, buffer: &mut [u8]) -> Result<usize, TransportIoError> {
        ...
        for byte in &mut buffer[..count] { *byte = wire.readable.pop_front().unwrap(); }
        Ok(count)
    }
...
fn fixture() -> (Runtime, ActuatorId) {
    ...
    runtime.register_transport(ResourceId::new(1),
        Box::new(Fake(Rc::new(RefCell::new(Wire::default()))))).unwrap();
    ...
    runtime.command(Command::RegisterMetakon(MetakonInstrumentConfig {
        definition: DataInstrumentDefinition {schema_version:1,id:instrument,
            name:"ACK fixture".into(),parameters:vec![DataParameterDefinition {
                id:parameter,name:"power".into(),value_spec:ValueSpec::Float{min:0.0,max:100.0},
                unit:Unit::PERCENT,access:AccessMode::ReadWrite,role:ParameterRole::Actuator,
                write_effect:WriteEffect::OutputAffecting,operation:KnownOperation::Output,scale:1.0,
            }]},
        binding: MetakonBinding {resource:ResourceId::new(1),device:15,channel:0,
            binding_generation:1,mapping_revision:1,expected_output_unit:Some(Unit::PERCENT)},
        history_capacity:2,
    })).unwrap();
    let actuator = ActuatorId::new(instrument,parameter);
    runtime.command(Command::Output {actuator,at:Duration::ZERO,
        command:OutputCommand::BindProfile(SafeProfile {min:0.0,max:100.0,safe_value:0.0,
            max_lease:Duration::from_secs(5),max_proposal_ttl:Duration::from_secs(1),
            required_evidence:EvidenceLevel::Acknowledgement})}).unwrap();
    runtime.command(Command::Output {actuator,at:Duration::ZERO,
        command:OutputCommand::RequestSafe}).unwrap();
    runtime.command(Command::QueueMetakonOutput {actuator,at:Duration::ZERO,
        queue_ttl:Duration::from_secs(1),timeout:Duration::from_millis(100)}).unwrap();
...
fn actual_metakon_ack_is_trusted_protocol_ack_without_readback_fact() {
    ...
    let CommandResult::Output(OutputResult::Lease(lease)) = runtime.command(Command::Output {
        actuator,at,command:OutputCommand::Acquire {owner:OutputOwner::Manual(1),
            lifetime:Duration::from_secs(1)},
    }).unwrap() else { panic!() };
    runtime.command(Command::Output {actuator,at,
        command:OutputCommand::Propose(OutputProposal {lease,value:Value::Float(40.0),
            unit:Unit::PERCENT,ttl:Duration::from_millis(200)})}).unwrap();
    runtime.command(Command::QueueMetakonOutput {actuator,at,
        queue_ttl:Duration::from_secs(1),timeout:Duration::from_millis(100)}).unwrap();
    ...
    assert!(facts.iter().any(|fact| matches!(fact,
        RecordingFact::Output {stage:OutputStage::Acknowledged,
